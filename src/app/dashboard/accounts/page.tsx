...
export default async function AccountsPage() {
  ...
  const assets = accounts.filter(a => a.type === "ASSET").map(a => a.balance).reduce((a, c) => Number(a) + Number(c), 0);
  const liabilities = accounts.filter(a => a.type === "LIABILITY").map(a => a.balance).reduce((a, c) => Number(a) + Number(c), 0);
